...
# Multi-head Legendre converlution layer
...
# Integrate transformer model with Legendre convolution and attention mechanism but no feature distillation
class IntegratedModelNoDistillation(nn.Module):
    def __init__(self, image_feature_dim, gnn_feature_dim, transformer_dim, ff_hidden_dim, num_heads, num_transformer_blocks, K, heads, translation_means, translation_stds):
        super(IntegratedModelNoDistillation, self).__init__()
    ...
    def forward(self, images, adj, object_ids, img_ids):
        ...
        transformer_input = rearrange(img_features_flat, 'b n d -> n b d')
...
def train_model(root_dir, epochs=10, K=3, heads=4, num_transformer_blocks=4, num_heads=8):
    ...
    model = IntegratedModelNoDistillation(image_feature_dim=256, gnn_feature_dim=256, transformer_dim=256, ff_hidden_dim=1024, 
